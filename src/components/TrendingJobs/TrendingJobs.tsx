...
const filterTabs = ['All Jobs', 'Technology', 'Marketing', 'Design', 'Business', 'Finance', 'Healthcare', 'Data Science'];

const statsItems = [
  { icon: 'bi-graph-up-arrow', label: 'High Demand', sub: 'Jobs with high market demand' },
  { icon: 'bi-buildings', label: 'Top Companies', sub: 'Opportunities at leading companies' },
  { icon: 'bi-cash-coin', label: 'Competitive Salaries', sub: 'Best pay for your skills' },
  { icon: 'bi-rocket-takeoff', label: 'Career Growth', sub: 'Build a successful career' },
];
    ...
    <section className="trending-section section-padding" id="trending" aria-label="Trending jobs">
      <div className="container">
        ...
        <div className="trending-header text-center mb-4">
          ...
          <p className="trending-subtitle">Explore the most in-demand jobs and kick-start your career today</p>
          <div className="trending-divider">
            <span></span><i className="bi bi-circle-fill"></i><span></span>
          </div>
        </div>

        {/* ── Filter tabs ── */}
        <div className="trending-filters mb-4" role="tablist" aria-label="Job category filters">
          {filterTabs.map((tab) => (
            <button
              key={tab}
              role="tab"
              aria-selected={activeFilter === tab}
              className={`trend-filter-btn${activeFilter === tab ? ' active' : ''}`}
              onClick={() => setActiveFilter(tab)}
              id={`filter-tab-${tab.toLowerCase().replace(/\s+/g, '-')}`}
            >
              {tab === 'All Jobs' && <i className="bi bi-grid-fill me-2"></i>}
              {tab === 'Technology' && <i className="bi bi-code-slash me-2"></i>}
              ...
              {tab === 'Finance' && <i className="bi bi-currency-dollar me-2"></i>}
              {tab === 'Healthcare' && <i className="bi bi-heart-pulse me-2"></i>}
              {tab === 'Data Science' && <i className="bi bi-database me-2"></i>}
              {tab}
            </button>
          ))}
        </div>
